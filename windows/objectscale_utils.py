...
class objectscale_utility:
    ...
    intall_micro_node_command = ['helm',
                                 ...
                                 'ecs/ecs-cluster',
                                 '--set',
                                 'global.registry=objectscale',
                                 '--generate-name',
                                 '--set',
                                 'storageServer.persistence.size=100Gi',
                                 '--set',
    ...
    def uninstall_objectscale(self):
        ...
        self.start_minikube_if_stoppped()
        result = self.run_command_get_all_output(self.list_all_charts, shell=True).lower()
    ...
    def install_objectscale(self, token: str, PATH=os.getenv('PATH')):
        ...
        if result.find('objectscale-helm') == -1:
            ...
            os.system(self.install_objectscale_command)
            os.system(self.intall_micro_node_command)
            return
    ...
    @staticmethod
    def run_command_get_all_output(command: str, Shell=True, stderr=subprocess.STDOUT) -> str:
        output = ''
        try:
